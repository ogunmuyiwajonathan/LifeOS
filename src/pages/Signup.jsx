...
export default function Signup() {
  ...
  const { signUp, signInWithGoogle, isAuthenticated, profile } = useAuthStore()
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setTimeout(() => navigate(result.redirectTo ?? '/onboarding', { replace: true }), 550)
    } catch (error) {
    ...
    >
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-[#8AA89A] mt-1">Start taking control of your life</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Full Name" className={fieldBaseClass} />
            <AnimatePresence>
              {errors.fullName && <motion.p {...errorShake} className="text-sm text-[#EF4444] mt-1">{errors.fullName}</motion.p>}
            </AnimatePresence>
          </div>

          <div>
            <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" className={fieldBaseClass} />
            <AnimatePresence>
              {errors.email && <motion.p {...errorShake} className="text-sm text-[#EF4444] mt-1">{errors.email}</motion.p>}
            </AnimatePresence>
          </div>
          ...
          <div>
            ...
            <div className="mt-2">
              <div className="w-full h-1.5 rounded-full bg-[#152018] overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
              ...
              <p className="text-xs text-[#8AA89A] mt-1">Strength: {strengthLabels[strength]}</p>
            </div>
            <AnimatePresence>
              {errors.password && <motion.p {...errorShake} className="text-sm text-[#EF4444] mt-1">{errors.password}</motion.p>}
            </AnimatePresence>
          </div>
          ...
          <div>
            ...
            <AnimatePresence>
              {errors.confirmPassword && (
                <motion.p {...errorShake} className="text-sm text-[#EF4444] mt-1">{errors.confirmPassword}</motion.p>
              )}
            </AnimatePresence>
          </div>
          ...
          <Button
            ...
            className={`w-full h-11 relative overflow-hidden rounded-xl text-white group ${
          ...
          <AnimatePresence>
            {submitError && <motion.p {...errorShake} className="text-sm text-[#EF4444] text-center">{submitError}</motion.p>}
          </AnimatePresence>
          {successMessage && <p className="text-sm text-[#8AA89A] text-center">{successMessage}</p>}
        </form>

        <p className="text-sm text-[#8AA89A] text-center mt-5">
          Already have an account?{' '}
          <Link to="/login" className="text-white hover:text-[#059669] transition-colors">
            Sign in
          </Link>
        </p>
      </motion.div>
